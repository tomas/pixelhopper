...
/* set window size */
SOKOL_APP_API_DECL void sapp_set_window_size(int width, int height);

SOKOL_APP_API_DECL bool sapp_window_is_visible();
SOKOL_APP_API_DECL void sapp_show_window();
SOKOL_APP_API_DECL void sapp_hide_window();
...
static void _sapp_get_time(struct timespec * ts) {
  clock_gettime(CLOCK_MONOTONIC, ts);
}

static double _sapp_get_timediff(struct timespec start) {
...
static void _sapp_get_time(struct timeval * ts) {
  gettimeofday(ts, NULL);
}

static double _sapp_get_timediff(struct timeval start) {
...
static struct mouse_click last_click = { -1, -1, -1, { 0, 0 } };
static double click_time_diff;
...
static bool is_double_click(int type, int x, int y) {
  ...
  if (last_click.y != -1 && y == last_click.y && type == last_click.type) {
    ...
    click_time_diff = _sapp_get_timediff(last_click.ts);

    // and toggle the flag if it took less than 0.4 secs
    res = click_time_diff < DOUBLE_CLICK_TIME;
  }
  ...
  _sapp_get_time(&last_click.ts);

  return res;
}

_SOKOL_PRIVATE bool _sapp_call_event(sapp_event* e) {

    if (e->type == SAPP_EVENTTYPE_MOUSE_DOWN) {
      if (is_double_click(e->mouse_button, e->mouse_x, e->mouse_y)) {
        e->num_touches = ++click_count;
      } else {
        e->num_touches = click_count = 1; // not double click. reset count
      }
    }
...
_SOKOL_PRIVATE sapp_desc _sapp_desc_defaults(const sapp_desc* desc) {
    ...
    res.fixed = _sapp_def(res.fixed, false);
    res.start_hidden = _sapp_def(res.start_hidden, false);
    res.borderless = _sapp_def(res.borderless, false);
    res.non_resizable = _sapp_def(res.non_resizable, false);
    res.wait_for_events = _sapp_def(res.wait_for_events, false);
...
_SOKOL_PRIVATE void _sapp_macos_show_window(void) {
    @autoreleasepool {
    [_sapp.macos.window orderFront:nil];
    } // autoreleasepool
}

_SOKOL_PRIVATE void _sapp_macos_hide_window(void) {
    @autoreleasepool {
...
_SOKOL_PRIVATE void _sapp_macos_set_window_size(int width, int height) {
    NSRect contentRect = [_sapp.macos.window contentRectForFrameRect:[_sapp.macos.window frame]];
    contentRect.origin.y += contentRect.size.height - height;
    contentRect.size = NSMakeSize(width, height);
    [_sapp.macos.window setFrame:[_sapp.macos.window frameRectForContentRect:contentRect]
                        display:YES];
}
...
_SOKOL_PRIVATE void _sapp_macos_poll_events() {
    @autoreleasepool {
...
_SOKOL_PRIVATE void _sapp_macos_wait_for_events() {
    @autoreleasepool {
...
- (void)drawRect:(NSRect)rect {
    ...
    if (&_sapp.desc->wait_for_events)
        _sapp_macos_wait_for_events();
    else
...
_SOKOL_PRIVATE void _sapp_x11_init_extensions(void) {
    ...
    _sapp.x11.MOTIF_WM_HINTS          = XInternAtom(_sapp.x11.display, "_MOTIF_WM_HINTS", False);
    ...
    if (_sapp.drop.enabled) {
        ...
        _sapp.x11.xdnd.text_moz_url     = XInternAtom(_sapp.x11.display, "_NETSCAPE_URL", False);
    }

    if (_sapp.clipboard.enabled) {
        _sapp.x11.CLIPBOARD = XInternAtom(_sapp.x11.display, "CLIPBOARD", False);
        _sapp.x11.CLIPBOARD_TARGETS = XInternAtom(_sapp.x11.display, "TARGETS", False);
    }
...
_SOKOL_PRIVATE void _sapp_x11_create_window(Visual* visual, int depth) {
    ...
    uint32_t wamask = CWBorderPixel | CWColormap | CWEventMask;
    ...
    if (!_sapp.desc.fixed && _sapp.desc.borderless) {
        ...
        XChangeProperty(_sapp.x11.display, _sapp.x11.window,
                        _sapp.x11.MOTIF_WM_HINTS,
                        _sapp.x11.MOTIF_WM_HINTS, 32,
                        PropModeReplace,
...
int _sapp_x11_get_clipboard_selection(const char *fmtname) {
  char *result;
  unsigned long ressize, restail;
  int resbits;

  Atom fmtid  = XInternAtom(_sapp.x11.display, fmtname, False),
       propid = XInternAtom(_sapp.x11.display, "XSEL_DATA", False),
       incrid = XInternAtom(_sapp.x11.display, "INCR", False);

  Window owner = XGetSelectionOwner(_sapp.x11.display, _sapp.x11.CLIPBOARD);
  if (owner == _sapp.x11.window) return 1; // we already own the clipboard!

  XEvent event;
  XConvertSelection(_sapp.x11.display, _sapp.x11.CLIPBOARD, fmtid, propid, _sapp.x11.window, CurrentTime);

  do {
    XNextEvent(_sapp.x11.display, &event);
  } while (event.type != SelectionNotify || event.xselection.selection != _sapp.x11.CLIPBOARD);

  if (event.xselection.property) {
    XGetWindowProperty(_sapp.x11.display, _sapp.x11.window, propid, 0, LONG_MAX/4, False, AnyPropertyType, &fmtid, &resbits, &ressize, &restail, (unsigned char**)&result);

    if (fmtid == incrid) printf("Buffer is too large and INCR reading is not implemented yet.\n");

    strncpy(_sapp.clipboard.buffer, result, ressize);
    _sapp.clipboard.buffer[ressize] = '\0';
...
char * _sapp_x11_get_clipboard_string() {
  int res = _sapp_x11_get_clipboard_selection("UTF8_STRING") || _sapp_x11_get_clipboard_selection("STRING");
  return res ? _sapp.clipboard.buffer : "";
}

void _sapp_x11_set_clipboard_string(char * buf) {
...
_SOKOL_PRIVATE bool _sapp_x11_parse_dropped_files_list(const char* src) {
    ...
    */
    ...
    while (0 != (src_chr = *src++)) {
        ...
        if (is_url && src_count == 0) { // URL title, skip chars until reaching \n
          if (src_chr == '\n') { // reached the end, toggle flag back
            is_url = false;
          }
          continue;
        }
        ...
        /* check leading 'file://' or http:// */
        if (!is_url && src_count <= 7) {

            if (((src_count == 1) && (src_chr == 'h')) ||
                ((src_count == 2) && (src_chr == 't')) ||
                ((src_count == 3) && (src_chr == 't')) ||
                ((src_count == 4) && (src_chr == 'p'))) {

                dst_chr = src_chr;
                if (src_count == 4) is_url = true;

            } else if (
                ((src_count == 1) && (src_chr != 'f')) ||
                ...
                SAPP_LOG("sokol_app.h: dropped file URI doesn't start with file:// or http");
                err = true;
                break;
    ...
    switch (event->type) {
            ...
            if (event->xclient.message_type == _sapp.x11.WM_PROTOCOLS) {
                ...
                for (uint32_t i = 0; i < count; i++) {
                    if (formats[i] == _sapp.x11.xdnd.text_uri_list) {
                        ...
                        break;
                    } else if (formats[i] == _sapp.x11.xdnd.text_moz_url) {
                        _sapp.x11.xdnd.format = _sapp.x11.xdnd.text_moz_url;
                        break;
                    }
                }
                if (is_list && formats) {
        ...
        case SelectionNotify:
            ...
            if (event->xselection.property == _sapp.x11.xdnd.XdndSelection) {
                ...
                Atom TARGETS = XInternAtom(_sapp.x11.display , "TARGETS", False);
                Atom UTF8_STRING = XInternAtom(_sapp.x11.display, "UTF8_STRING", False);
                Atom target = None;
                ...
                unsigned char* data;
                unsigned long count;
                XGetWindowProperty(_sapp.x11.display, _sapp.x11.window, _sapp.x11.CLIPBOARD, 0, LONG_MAX, False, AnyPropertyType, &actualType, &actualFormat, &count, &bytesAfter, &data);

                if (event->xselection.target == TARGETS) {
                    Atom* list = (Atom*)data;
                    ...
                    if (target != None) {
                        printf("Converting selection\n");
                        XConvertSelection(_sapp.x11.display, _sapp.x11.CLIPBOARD, target, _sapp.x11.CLIPBOARD, _sapp.x11.window, CurrentTime);
                    }
                } else if (event->xselection.target == target) {
                    printf("Got selection data!\n");
        ...
        case SelectionClear:
            printf("SelectionClear: lost ownership of clipboard selection.\n");
            break;

        case SelectionRequest:
            printf("SelectionRequest!\n");

            // Atom CLIPBOARD = XInternAtom(_sapp.x11.display , "CLIPBOARD", False);
            Atom TARGETS = XInternAtom(_sapp.x11.display , "TARGETS", False);
            Atom UTF8_STRING = XInternAtom(_sapp.x11.display, "UTF8_STRING", False);
            // Atom XA_STRING = XInternAtom(_sapp.x11.display, "XA_STRING", False);
            // Atom PNG_IMAGE   = XInternAtom(_sapp.x11.display, "image/png", False);
            // Atom target = None;

            // XSelectionRequestEvent * request = &event->xselectionrequest;
            // if (XGetSelectionOwner(_sapp.x11.display, _sapp.x11.CLIPBOARD) == _sapp.x11.window && event->xselectionrequest.selection == _sapp.x11.CLIPBOARD) {
            //     if (event->xselectionrequest.target == _sapp.x11.CLIPBOARD_TARGETS && event->xselectionrequest.property != None) {
            //         XSelectionEvent reply;
            //         // XSelectionEvent reply = {0};
            //         _sapp_clear(&reply, sizeof(reply));
            //         reply.type      = SelectionNotify;
            //         reply.serial    = event->xselectionrequest.serial;
            ...
            //         XSendEvent(_sapp.x11.display, event->xselectionrequest.requestor, 0, 0, (XEvent*)&reply);
            //         XFlush(_sapp.x11.display);
            //     }
            // }

            XSelectionRequestEvent request = event->xselectionrequest;
            if (XGetSelectionOwner(_sapp.x11.display, _sapp.x11.CLIPBOARD) == _sapp.x11.window && request.selection == _sapp.x11.CLIPBOARD) {

                if (request.target == TARGETS && request.property != None) {
                    printf("sending targets\n");
                    XChangeProperty(request.display, request.requestor, request.property, XA_ATOM, 32, PropModeReplace, (unsigned char*)&UTF8_STRING, 1);
                } else if (request.target == UTF8_STRING && request.property != None) {
                    printf("sending data\n");
                    XChangeProperty(request.display, request.requestor, request.property, request.target, 8, PropModeReplace, _sapp.clipboard.buffer, strlen(_sapp.clipboard.buffer));
                }

                XSelectionEvent sendEvent;
                _sapp_clear(&sendEvent, sizeof(sendEvent));
                ...
                XSendEvent(_sapp.x11.display, request.requestor, 0, 0, (XEvent*)&sendEvent);
                // XFlush(_sapp.x11.display);
            }
...
_SOKOL_PRIVATE bool _sapp_x11_wait_for_event() {
    fd_set fds;
    const int fd = ConnectionNumber(_sapp.x11.display);
    int count = fd + 1;
...
SOKOL_APP_IMPL Display* sapp_x11_get_display(void) {
    SOKOL_ASSERT(_sapp.valid);
    #if defined(_SAPP_LINUX) // && !defined(_SAPP_GLX)
        return _sapp.x11.display;
    #else
        return 0;
    #endif
}
...
SOKOL_API_IMPL const char* sapp_get_clipboard_string(void) {
    ...
    #if defined(_SAPP_MACOS)
        return _sapp_macos_get_clipboard_string();asdasdasdasdasdasd
    #elif defined(_SAPP_EMSCRIPTEN)
        return _sapp.clipboard.buffer;
    #elif defined(_SAPP_WIN32)
...
SOKOL_API_IMPL bool sapp_window_is_visible() {
    #if defined(_SAPP_MACOS)
...
SOKOL_API_IMPL void sapp_show_window() {
    // if (sapp_window_is_visible(window))
    //     return;

    #if defined(_SAPP_MACOS)
        _sapp_macos_show_window();
    #elif defined(_SAPP_WIN32)
        _// TODO
    #elif defined(_SAPP_LINUX)
        _sapp_x11_show_window();
...
SOKOL_API_IMPL void sapp_hide_window() {
    // if (!sapp_window_is_visible(window))
    //     return;

    #if defined(_SAPP_MACOS)
        _sapp_macos_hide_window();
    #elif defined(_SAPP_WIN32)
        _// TODO
    #elif defined(_SAPP_LINUX)
...
typedef glproc (*_getprocaddressfun)(const char*);

#if _SAPP_APPLE
static glproc getProcAddressNSGL(const char* procname) {
    CFBundleRef framework  = CFBundleGetBundleWithIdentifier(CFSTR("com.apple.opengl"));
    if (framework == NULL) return NULL;
    CFStringRef symbolName = CFStringCreateWithCString(kCFAllocatorDefault, procname, kCFStringEncodingASCII);
    GLFWglproc symbol      = CFBundleGetFunctionPointerForName(framework, symbolName);
    CFRelease(symbolName);
    return symbol;
}
#endif

SOKOL_API_IMPL void * sapp_gl_get_proc_address(const char* procname) {
#if defined(_SAPP_GLX)
    return _sapp_glx_getprocaddr(procname);
#elif defined(_SAPP_WIN32)
    return _sapp.wgl.GetProcAddress(procname);
#elif defined(_SAPP_APPLE)
    return (void*)getProcAddressNSGL(procname);
#endif
}
